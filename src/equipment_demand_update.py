...
def filter_data(df, equipment_list, category=20):
    '''
    ...
    Input: dataframe to filter, list of equipment types to keep for analysis, equipment category to filter out.
...
def add_features(df):
    ...
    df.loc[:, 'invoice_start_year'] = df['month'].dt.year
    df.loc[:, 'invoice_start_month'] = df['month'].dt.month
...
def get_month_lag(df, row):
    ...
    '''
    lag_date = row['month'] - 1 * MonthBegin()
    mask = (lag_date == df.loc[:, 'month']) & (row['product_type'] == df.loc[:, 'product_type']) & (row['rental_type'] == df.loc[:, 'rental_type'])
    lag_df = df.loc[mask, :]
    return lag_df
...
def get_month_avg(df, row):
    ...
    '''
    month = row['invoice_start_month']
    mask = (month == df.loc[:, 'invoice_start_month']) & (row['product_type'] == df.loc[:, 'product_type']) & (row['rental_type'] == df.loc[:, 'rental_type'])
    lag_df = df.loc[mask, :]
    return lag_df
...
def create_same_month_avg(df_agg, cutoff_date):
    ...
    '''
    df_agg_avg_month = df_agg.loc[df_agg['month'] < cutoff_date, :]
    df_agg_avg_month = df_agg_avg_month.groupby(['product_type', 'invoice_start_month', 'rental_type']).agg({'units_rented': 'mean', 'total_days_rented': 'mean'}).reset_index()
    for idx, row in df_agg.iterrows():
...
if __name__ == '__main__':
    ...
    # filter on category 20 (booms, scissors, etc), and top 30 equipment types that account for 95% of revenue (or top 17 equipment types that account for 80% of revenue).
    ...
    df_rev = filter_data(df_rev, top_30_equipment, 20)

    ## Drop and reformat columns
    cols_to_drop = ['Client Code', 'Client Region', 'CreditAmount', 'Client District', 'Client Category', 'Country', 'Equipment ID #', 'IsSubstitution', 'Description', 'Transaction ID #', 'IsRPO', 'NationalAcctCode', 'Sales Rep #', 'Sales Rep Name', 'Customer Name', 'Minimums', 'IsNationalAcct', 'IsSpecialPricing', 'IsContract', 'ContractPriceNo', 'CycleBillNo', 'Jobsite ZipCode', 'Branch ZipCode', 'CustomerCorpID', 'Customer #', 'Minimum Revenue', 'Minimum Book Revenue', 'ExcessMileageRevenue', 'UnclassifiedRevenue', 'ContractStartDate']
    ...
    # add features (rental_type, invoice start dates, units rented)
    ...
    df_agg = df_rev.groupby(['product_type', 'month', 'invoice_start_month', 'rental_type']).agg({'rental_revenue':'sum', 'total_days_rented': 'sum', 'units_rented':'sum'}).reset_index()
    ...
    df_agg.sort_values('month', inplace=True)
    ...
    # remove February 2013 data as there is no prior month
    df_agg = df_agg.loc[df_agg['month'] > '2013-02-01', :]
    df_agg.reset_index(drop=True, inplace=True)
    ...
    df_agg = create_dummies('invoice_start_month', df_agg, 'invoice_start_month')
    ...
    df_agg = df_agg.loc[df_agg['month'] < '2018-05-01', :]
    ...
    models = [(linear, 'Linear'), (lasso, 'Lasso'), (rf, 'Random Forest'), (gbr, 'Gradient Boosting'), (catboost, 'CatBoost')]
    ...
    catboost_params = {'depth':[1,3,5,7,9],
          'iterations':[250,100,500,1000],
    ...
    catboost_gridsearch = GridSearchCV(CatBoostRegressor(),
                                    catboost_params,
                                    n_jobs=-1,
                                    verbose=True,
                                    cv=3)
    catboost_gridsearch.fit(X_train_std, y_train)
    print("best parameters catboost: {}".format(catboost_gridsearch.best_params_))

    catboost_best_model = catboost_gridsearch.best_estimator_
    catboost_best_model.fit(X_train_std, y_train)
    catboost_train_preds = catboost_best_model.predict(X_train_std)
    catboost_test_preds = catboost_best_model.predict(X_test_std)
    catboost_train_rmse = np.sqrt(mean_squared_error(y_train, catboost_train_preds))
    catboost_test_rmse = np.sqrt(mean_squared_error(y_test, catboost_test_preds))
    print('best catboost model Train RMSE score: {}'.format(catboost_train_rmse))
    print('best catboost model Test RMSE score: {}'.format(catboost_test_rmse))
    ...
    mse_same_month_train = mean_squared_error(y_train, y_pred_train_same_month)
    rmse_same_month_train = np.sqrt(mse_same_month_train)
    mse_same_month_test = mean_squared_error(y_test, y_pred_test_same_month)
    rmse_same_month_test = np.sqrt(mse_same_month_test)
    ...
    features_ols = df_agg_train.drop(['units_rented', 'month', 'rental_revenue', 'total_days_rented', 'avg_price_per_day'], axis=1).columns
    coeffs_by_feature = sorted(zip(coeffs_ols, features_ols), reverse=True)

    # look at coefficients from lasso model
    features_lasso = df_agg_train.drop(['units_rented', 'month', 'rental_revenue', 'total_days_rented', 'avg_price_per_day'], axis=1).columns
